...
from langchain_community.document_loaders.csv_loader import CSVLoader # 【問題1】CSVのFAQデータ読み込み用
...
# RAG参照用のデータソース系
# ==========================================
RAG_TOP_FOLDER_PATH = str(BASE_DIR / "data" / "rag")
...
DB_ALL_PATH = str(BASE_DIR / ".db_all")
DB_COMPANY_PATH = str(BASE_DIR / ".db_company")
...
DB_SERVICE_PATH = str(BASE_DIR / ".db_service")
DB_CUSTOMER_PATH = str(BASE_DIR / ".db_customer")
DB_FAQ_PATH = str(BASE_DIR / ".db_faq")

DB_NAMES = {
    DB_COMPANY_PATH: str(BASE_DIR / "data" / "rag" / "company"),
    DB_SERVICE_PATH: str(BASE_DIR / "data" / "rag" / "service"),
    DB_CUSTOMER_PATH: str(BASE_DIR / "data" / "rag" / "customer"),
    DB_FAQ_PATH: str(BASE_DIR / "data" / "rag" / "faq")   # 【問題1】エージェントに渡すツールの追加
}
...
SEARCH_COMPANY_INFO_TOOL_DESCRIPTION = "よくある質問（FAQ）データベースに無さそうな回答の場合、自社「株式会社EcoTee」に関する情報を参照したい時に使う"
SEARCH_SERVICE_INFO_TOOL_NAME = "search_service_info_tool"
SEARCH_SERVICE_INFO_TOOL_DESCRIPTION = "よくある質問（FAQ）データベースに無さそうな回答の場合、自社サービス「EcoTee」に関する情報を参照したい時に使う"
SEARCH_CUSTOMER_COMMUNICATION_INFO_TOOL_NAME = "search_customer_communication_tool"
SEARCH_CUSTOMER_COMMUNICATION_INFO_TOOL_DESCRIPTION = "よくある質問（FAQ）データベースに無さそうな回答の場合、顧客とのやりとりに関する情報を参照したい時に使う"
SEARCH_FAQ_INFO_TOOL_NAME = "search_faq_info_tool"  # 【問題1】エージェントに渡すツールの追加
SEARCH_FAQ_INFO_TOOL_DESCRIPTION = "よくある質問（FAQ）データベースから回答を検索する際に使う。基本的な質問や一般的な質問の場合に優先的に使用する"
...
# Slack連携関連
# ==========================================
EMPLOYEE_FILE_PATH = str(BASE_DIR / "data" / "slack" / "従業員情報.csv")
INQUIRY_HISTORY_FILE_PATH = str(BASE_DIR / "data" / "slack" / "問い合わせ対応履歴.csv")
CSV_ENCODING = "utf-8-sig"
SLACK_CHANNEL_ID = "!channel" # 課題外。メンション先がいない場合はチャンネルをメンションする
...
"""
# 【問題1】メンション先の選定理由の追加
# 【課題外】メンション先がいない場合はチャンネルをメンションする。
# また、Agentが「Now I will」などの英語説明を出力してLLMがParseErrorを発生させないようにする対応。
SYSTEM_PROMPT_NOTICE_SLACK = """
    ...
    # 重要な実行ルール
    - 説明文、思考プロセス、コメントは一切出力しないでください
    - 「I have obtained」「Now I will」「Here is」などの英語説明は絶対に出力しないでください
    - 「### Message Generation」「### Final Message Structure」などの見出しは絶対に出力しないでください
    - メッセージ内容を別途表示する必要はありません
    - チャンネルIDを取得したら、即座にSlack送信ツールでメッセージを送信してください
    - 送信前の確認や説明は不要です
    ...
    - 【回答・対応案】について、以下の条件に従って生成してください。
        ...
        - 各対応案は、顧客にとって実用的で、すぐに実行可能な内容にしてください。
        - 根拠には、従業員の過去の対応履歴や専門知識を具体的に引用してください。
...
# 次のアクション
上記のメッセージフォーマットに従って内容を生成し、説明なしで即座にSlack送信ツール（send_message）を使用してメッセージを送信してください。
"""
