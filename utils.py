...
def create_rag_chain(db_name, use_streaming=True):
    """
    引数として渡されたDB内を参照するRAGのChainを作成

    Args:
        db_name: RAG化対象のデータを格納するデータベース名
        use_streaming: ストリーミングを使用するか（デフォルト: True）
    """
    ...
    # ストリーミングの有無に応じてLLMを選択
    llm = st.session_state.llm if use_streaming else st.session_state.llm_no_stream
...
def run_faq_chain(param):
    ...
    Returns:
        LLMからの回答
    """
    # FAQデータ参照に特化したChainを実行してLLMからの回答取得
    ai_msg = st.session_state.faq_chain.invoke({"input": param, "chat_history": st.session_state.chat_history})
...
def execute_agent_or_chain(chat_message):
    ...
    if st.session_state.agent_mode == ct.AI_AGENT_MODE_ON:
        ...
        # マルチスレッド環境でのStreamlitCallbackHandlerエラーを回避するため、コールバックなしで実行
...
def notice_slack(chat_message):
    ...
    """
    logger = logging.getLogger(ct.LOGGER_NAME)
    logger.debug("notice_slack 関数を開始")

    # Slack通知用のAgent Executorを作成
    # StreamlitCallbackHandlerのエラーを防ぐため、ストリーミング無効のLLMを使用
    toolkit = SlackToolkit()
    tools = toolkit.get_tools()
    agent_executor = initialize_agent(
        llm=st.session_state.llm_no_stream,
        tools=tools,
        agent=AgentType.STRUCTURED_CHAT_ZERO_SHOT_REACT_DESCRIPTION
        # handle_parsing_errors=True  # パースエラーをキャッチして再試行
    )
    ...
    # StreamlitCallbackHandlerエラーを回避するため、ストリーミング無効のLLMを使用
    employee_id_response = st.session_state.llm_no_stream.invoke(messages)
    ...
    if not target_employees:
        logger.info(f"ユーザー入力：{chat_message} \n問い合わせ内容と関連性が高い従業員情報が存在しません。チャンネルをメンションします。")
        slack_ids = []
        slack_id_text = "チャンネル"
        slack_ids.append(ct.SLACK_CHANNEL_ID)
    else:
    ...
    # デバッグ用：プロンプトメッセージの内容をログに出力
    logger.info("=" * 80)
    logger.info("Slack通知用プロンプトメッセージ:")
    logger.info("-" * 80)
    ...
    try:
        ...
        logger.info("Agent Executor 実行成功")
        logger.info(f"結果: {result}")
    except Exception as e:
        logger.error("=" * 80)
        logger.error("Agent Executor 実行中にエラーが発生しました")
        ...
        if hasattr(e, 'llm_output'):
            logger.error(f"LLM出力: {e.llm_output}")
        if hasattr(e, 'observation'):
            logger.error(f"観測結果: {e.observation}")
...
def get_datetime():
    ...
    # 元のコード。エラーが発生：「'locale' codec can't encode character '\u5e74' in position 2: encoding error」
    # now_datetime = dt_now.strftime('%Y年%m月%d日 %H:%M:%S')
    # Claude4.5による修正。Localeエラーは出なくなったが、agent_executorで以下のエラーが発生。
    # [ERROR] 2025-10-01 12:24:12,179 line 120, in <module>, session_id=99c47cf618454267a843dfe16309b1bc: ユーザー入力に対しての処理に失敗しました。
    # 出力パースエラーが発生しました。**このエラーをエージェントに返して再試行させるには、AgentExecutorにhandle_parsing_errors=Trueを渡してください。
    # エラー内容：LLM出力をパースできませんでした：
    # now_datetime = f'{dt_now.year}年{dt_now.month:02d}月{dt_now.day:02d}日 {dt_now.hour:02d}:{dt_now.minute:02d}:{dt_now.second:02d}'
    
    # 一旦、yyyy/mm/dd/hh:mm:ssの形式に変更
    now_datetime = dt_now.strftime('%Y/%m/%d/ %H:%M:%S')
    return now_datetime
