...
def initialize_agent_executor():
    ...
    # ツール用のLLM（ストリーミング無効）- StreamlitCallbackHandlerエラーを回避
    st.session_state.llm_no_stream = ChatOpenAI(model_name=ct.MODEL, temperature=ct.TEMPERATURE, streaming=False)

    # 各Tool用のChainを作成（ストリーミング無効のLLMを使用）
    st.session_state.customer_doc_chain = utils.create_rag_chain(ct.DB_CUSTOMER_PATH, use_streaming=False)
    st.session_state.service_doc_chain = utils.create_rag_chain(ct.DB_SERVICE_PATH, use_streaming=False)
    st.session_state.company_doc_chain = utils.create_rag_chain(ct.DB_COMPANY_PATH, use_streaming=False)
    st.session_state.faq_chain = utils.create_rag_chain(ct.DB_FAQ_PATH, use_streaming=False) # 【問題1】エージェントに渡すツールの追加
    st.session_state.rag_chain = utils.create_rag_chain(ct.DB_ALL_PATH)

    # Web検索用のToolを設定するためのオブジェクトを用意
    search = SerpAPIWrapper()

    # 例：FAQ自動応答用のAgent機能を追加
    # FAQ検索用のToolを設定するためのオブジェクトを用意
    # Toolの追加
    tools = [
        # 会社に関するデータ検索用のTool
        Tool(
        ...
        # FAQ検索用のTool 【問題1】エージェントに渡すツールの追加
        Tool(
            name=ct.SEARCH_FAQ_INFO_TOOL_NAME,
            func=utils.run_faq_chain,
    ...
    # Agent Executorの作成
    # マルチスレッド環境でのStreamlitCallbackHandlerエラーを回避するため、ストリーミング無効のLLMを使用
